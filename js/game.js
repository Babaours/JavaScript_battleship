...
(function () {
    "use strict";
/*var sea = new Audio("sounds/sea.ogg");
    ...
    var game = {
        ...
        rightClick: function(e)
        {
            ...
            ship.dom.style.transform = window.rightClick ? "rotate(90deg)" : "rotate(0deg)";
            ...
            if(ship.getLife()% 2 == 0)
                {
                    ...
                    {    console.log('okidoki');
                        ship.dom.style.top = "" + (utils.eq(e.target.parentNode)) * utils.CELL_SIZE - (630 + this.players[0].activeShip * 60) + "px";
        ...
        handleMouseMove: function (e) {
            ...
            if (this.getPhase() === this.PHASE_INIT_PLAYER && e.target.classList.contains('cell')) 
            {
                ...
                if(ship.getLife()% 2 == 0)
                {
                    ...
                    if(window.rightClick)
                    {    console.log('bonourjesuisunconsolelog');
                        ship.dom.style.top = "" + (utils.eq(e.target.parentNode)) * utils.CELL_SIZE - (630 + this.players[0].activeShip * 60) + "px";
                        ship.dom.style.left = "" + utils.eq(e.target) * utils.CELL_SIZE - Math.floor(ship.getLife() / 2) * utils.CELL_SIZE + 30 + "px";
                    }
        ...
        fire: function (from, col, line, callback) {
            ...
            target.receiveAttack(col, line, function (hasSucceed) {
                var tir = new Audio("sounds/tir.ogg");
                var touch = new Audio("sounds/touch.ogg");
                var miss = new Audio("sounds/miss.ogg");
                tir.play();
        ...
        renderMiniMap: function () 
        {
            ...
            this.players[0].fleet.forEach(function(ship)
            {
                ...
                miniGrid.style.marginTop = "-210px";
            });
